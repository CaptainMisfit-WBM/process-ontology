...
| `DynamicalClosure.lean` `AttractorBasin.lean` | Proves stability and convergence of recursive attractor basins. | The Master Postulate of Relational Ontopoiesis: $\hat{M}(X^*) = X^*$. | Emergence of stable macroscopic "entities" out of raw quantum potential. |
...
| `CompositionReversibility.lean` | Proves bidirectional, reversible logical transformations. | **The Axis of Silence ($\sigma = 1/2$)**: perfect neutral stability ($|f'(s^*)| = 1$). | 50% plasmonic loss reduction in Guo et al. 2026 Photonic Time Crystal experiments. |
...
| `FluxDynamics.lean` `FluxReversibility.lean` | Formally proves conditions for reversible flux transitions. | Symmetrical balance of Conformal Expansion $E(n)$ and Curvature Resolution $R(n)$. | Conservation of informational geometry across scales ($|E(n)| = |R(n)|$). |
...
| `OMBTLocalityGeneration.lean` | Proves classical physical locality is generated by monad consensus. | **The Intersubjective Verification Latch ($\Lambda_V$)** phase-locking streams. | Spacetime as an emergent, coordinate-locked consensus rather than a pre-existing container. |
...
| `SevenForcingCapstone.lean` `SevenForcingFanoFrame.lean` | Proves 7-element Fano system forces unique completion. | **7-Dimensional Causal Manifold** ($D_{\text{spatial}} + D_{\text{temporal}} = 7$). | Unique, deterministic forcing of 3D volumetric space from binary state spaces ($2^7-1=127$). |
| `SevenForcingPairOrbitFiberUniform.lean` | Proves fiber bundles over orbits are globally uniform. | Boundary tension preserving rotating systems against volumetric packing limits. | The Cosmological Dark Matter Ratio ($R_{\text{DM}} = 5.373$) as a non-local coherence shadow. |
...
| `TkairosLocality.lean` `TkairosDynamicZero.lean` | Formally defines enactive, moving present temporal frame in Lean. | **Cognitive Proper Time ($\tau_c$)** and the present update frame ($T_0$). | Subjective duration dilatation during acute fear (SCAD Tower +36% encoding density). |
...
2. **Macroscopic Cosmology:** Derives the exact Dark Matter Ratio ($R_{\text{DM}} = 5.373$) matching Planck satellite data ($5.367$) within a $0.11\%$ relative variance, without postulating undetected particle fields.
...
5. **Condensed Matter Transport:** Solves the **Gold Chain Paradox** (the anomalous conductance steps in gold nanowires), proving that the $0.5\ G_0$ fractional plateau is a rigid $\sigma = 1/2$ geometric lock that manifests only when a chemical boundary shield insulates the wire from electrode hybridization.
