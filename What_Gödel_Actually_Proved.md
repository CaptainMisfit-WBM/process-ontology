...
## **A Unified Fixed-Point Analysis of Knowledge, Truth, and Self-Reference**
...
* [https://museums4all.org/participating-museums/](https://museums4all.org/participating-museums/)$\\text{Prov}(x)$: The formula with Gödel number $x$ is provable within the object-language framework.
...
6. $$P(\\text{Know}(\\ulcorner K \\urcorner) \\mid \\text{Tr}(\\ulcorner K \\urcorner)) \= \\frac{P(\\text{Know}(\\ulcorner K \\urcorner) \\wedge \\text{Tr}(\\ulcorner K \\urcorner))}{P(\\text{Tr}(\\ulcorner K \\urcorner))}$$
