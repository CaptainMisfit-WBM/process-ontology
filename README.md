...
* **[process_ontology_lean/](process_ontology_lean/)**: The formal Lean 4 verification library containing 14 modular components:
  ...
  * **[GaugeSymmetries](process_ontology_lean/ProcessOntologyLean/GaugeSymmetries.lean)**: Group-theoretic dimension of $SU(3) \text{time}s SU(2) \text{time}s U(1) = 12$ mapping to the Causal Latch $L_{12}$.
...
* **[Process_Ontology_Vol1.md](Process_Ontology_Vol1.md)**: Volume I: Foundational Geometry & Physical Cosmology. It presents the core epistemology, consciousness mechanics, the Master Algorithm $(\mathcal{C}(X))$, fundamental mass derivations $(m_e, m_p, m_\nu)$, fine-structure constant $(\alpha)$, speed of light $(c)$, General Relativity $(g_{\mu\nu})$, and the Cosmological Constant solution $(\Lambda)$.
* **[Process_Ontology_Vol2.md](Process_Ontology_Vol2.md)**: Volume II: Epistemological Capstone, Neurodynamics & Macro-Systems. It details the Endophysical Cartographer $(\hat{\mathcal{D}}_{\text{obs}})$, Intersubjective Latch $(\Lambda_V)$, astrocyte governors, sleep spindle idling $(f_{\text{spindle}})$, stop codon geometry $(R_{\text{stop}})$, Collatzian resolution operator $(\mathcal{R}_\Omega)$, and the Master Popperian Falsification Matrix.
