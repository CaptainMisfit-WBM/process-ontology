...
* **[Circular_Theory_of_Ego_and_Awareness.md](Circular_Theory_of_Ego_and_Awareness.md)**: The unified master monograph of consciousness and mathematical cognitive mechanics. Synthesizes the qualitative Dualist Engine (Ego/Awareness) with exact Process Ontology formalisms ($D(X), \mathcal{Y}^+/\mathcal{Y}^- = \phi, \mathcal{C}(X), \delta\_{\text{slip}}$). It models the self as a dynamic fixed point generated by the recursive flow of Dissonance, balanced by the dual forces of Ego (Assertion) and Awareness (Interrogation).
* **[What_Gödel_Actually_Proved.md](What_Gödel_Actually_Proved.md)**: The unified master monograph of epistemic incompleteness and formal self-reference. Proves the modal-epistemic collapse of internalist foundationalism ($P(\text{Know} \mid \text{Tr}) = 0$) and establishes its exact topological isomorphism to Process Ontology ($M(1) = \phi^{-2}$). It shows that an embedded observer cannot self-ground without generating a non-zero phase slip $(\delta\_{\text{slip}})$, which acts as the thermodynamic engine driving the arrow of time.
: Formal mathematical proofs compiling the logic of the Circular Theory and the Gödelian limits of internal measurement.
...
All 2,182 compilation jobs build cleanly with 0 errors and 0 `sorry` statements. Every geometric derivation, symmetry group dimension ($L\_{12} = 12$), scaling relation ($\phi, P$), and zero-parameter physical constant ($\alpha^{-1}$) is fully closed and verified in Lean v4.33.1.
